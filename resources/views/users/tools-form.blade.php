...
@section('content')
    ...
    <form action="{{ route('tools-form-post', ['textObject' => $data]) }}" method="POST" enctype="multipart/form-data" class="w-2/3 flex flex-col bg-white items-center rounded p-4 mx-auto">
        ...
        {{-- window --}}
        <div class="flex flex-col md:flex-row items-center justify-center w-full mt-4">
            <label for="windows" class="block mb-2 text-sm  text-gray-900 font-semibold md:mr-4 md:w-1/4">Windows file</label>
            <div class="flex flex-row md:w-3/4">
                <input type="file" name="windows" id="windows" accept=".exe" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary focus:border-primary block w-3/4 p-2.5"  />
                <input type="text" name="version-windows" id="version-windows" value="{{ old('version-windows') }}" placeholder="Version" class="ml-4 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary focus:border-primary block w-1/4 p-2.5"/>
            </div>
        </div>
        {{-- macos --}}
        <div class="flex flex-col md:flex-row items-center justify-center w-full mt-4">
            <label for="mac" class="block mb-2 text-sm  text-gray-900 font-semibold md:mr-4 md:w-1/4">Dmg file</label>
            <div class="flex flex-row md:w-3/4">
                <input type="file" name="mac" id="mac" accept=".dmg" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary focus:border-primary block w-3/4 p-2.5"  />
                <input type="text" name="version-mac" id="version-mac" value="{{ old('version-mac') }}" placeholder="Version" class="ml-4 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary focus:border-primary block w-1/4 p-2.5"/>
            </div>
        </div>
